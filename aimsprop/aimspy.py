...
def parse_aimspy(
    ...
    tbfs = [
        int(re.match("%s/data/tbf_(\S+)/" % filepath, tbf).group(1))
        for tbf in tbf_files
    ]

    # obtain tbf data from general output
    for tbf in tbfs:
        os.system(
            "grep '@tbf%d\ ' %s/%s > %s/tbf%d.out"
            % (tbf, filepath, outfile, filepath, tbf)
        )

    # The files (these are standard textual output from PyMS)
    posfiles = {
        tbf: "%s/data/tbf_%04d/x%04d.xyz" % (filepath, tbf, tbf) for tbf in tbfs
    }
